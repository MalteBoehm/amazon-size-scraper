...
type DatabaseConfig struct {
	...
	DBName   string
	SSLMode  string
	URL      string // Full database connection URL
}
...
type LoggingConfig struct {
	Level    string
	Format   string
	LogHTML  bool   // Enable HTML dumping for debugging
	HTMLDir  string // Directory for HTML dump files
	DebugMode bool   // Enable debug mode with console logging instead of database writes
}
...
func Load() (*Config, error) {
	cfg := &Config{
		...
		Database: DatabaseConfig{
			Host:     getEnvOrDefault("DATABASE_HOST", getEnvOrDefault("DB_HOST", "localhost")),
			Port:     getIntOrDefault("DATABASE_PORT", getIntOrDefault("DB_PORT", 5432)),
			User:     getEnvOrDefault("DATABASE_USER", getEnvOrDefault("DB_USER", "postgres")),
			Password: getEnvOrDefault("DATABASE_PASSWORD", getEnvOrDefault("DB_PASSWORD", "")),
			DBName:   getEnvOrDefault("DATABASE_NAME", getEnvOrDefault("DB_NAME", "amazon_scraper")),
			SSLMode:  getEnvOrDefault("DATABASE_SSL_MODE", getEnvOrDefault("DB_SSL_MODE", "disable")),
		...
		Logging: LoggingConfig{
			...
			LogHTML:  getBoolOrDefault("SCRAPER_LOG_HTML", false),
			HTMLDir:  getEnvOrDefault("SCRAPER_HTML_LOG_DIR", "logs/html"),
