...
func (r *OutboxRepository) InsertWithTx(ctx context.Context, tx Tx, event *OutboxEvent) error {
	...
	if event.TargetStream == "" {
		event.TargetStream = constants.StreamProductLifecycle
	}

	now := time.Now()
	event.CreatedAt = now
	
	// Check if this is a PRODUCT_IGNORED event that should not be retried
	// Support both old and new event type names
	...
	if event.EventType == "PRODUCT_IGNORED" || event.EventType == "02B_PRODUCT_IGNORED" {
		...
		if err := json.Unmarshal(event.Payload, &payload); err == nil {
			if reason, ok := payload["reason"].(string); ok {
				// Don't retry if the product has no size table
	...
	// Only set NextRetryAt if retry is needed
	if event.NextRetryAt == nil && !shouldNotRetry {
	...
	_, err := tx.Exec(ctx, query,
		...
		event.ErrorMessage, event.CreatedAt, event.NextRetryAt,
	)
...
func (r *OutboxRepository) GetPending(ctx context.Context, limit int) ([]*OutboxEvent, error) {
	...
	if err != nil {
		...
		if err != nil && (err.Error() == "pq: password authentication failed for user \"postgres\"" ||
			...
			return nil, fmt.Errorf("database authentication failed - connection may have expired: %w", err)
		}
