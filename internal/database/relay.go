	...
	for {
		...
		case <-ticker.C:
			...
			if err := r.db.Ping(ctx); err != nil {
				r.logger.Error("database connection validation failed", 
					"error", err,
					"action", "skipping_cycle")
				// Skip this cycle to allow connection pool to refresh
				continue
			}
			...
			if err := r.processEvents(ctx); err != nil {
				...
				if isAuthError(err) {
					r.logger.Error("authentication error detected, waiting for connection refresh", 
						"error", err)
					// Wait a bit longer to allow connection pool to refresh
...
func (r *Relay) processEvents(ctx context.Context) error {
	...
	r.logger.Debug("attempting to fetch pending events", "batch_size", r.batchSize)
	
	events, err := r.outbox.GetPending(ctx, r.batchSize)
	if err != nil {
		// Add more detailed error logging
		r.logger.Error("database query failed in relay", 
			"error", err,
...
func (r *Relay) GetPendingCount(ctx context.Context) (int64, error) {
	...
	if err != nil {
		...
		if err != nil && (err.Error() == "pq: password authentication failed for user \"postgres\"" ||
			err.Error() == `pq: password authentication failed for user "postgres"`) {
			r.logger.Error("authentication failed in GetPendingCount", "error", err)
			return 0, fmt.Errorf("database authentication failed: %w", err)
		}
...
func (r *Relay) GetDeadLetterCount(ctx context.Context) (int64, error) {
	...
	if err != nil {
		...
		if err != nil && (err.Error() == "pq: password authentication failed for user \"postgres\"" ||
			err.Error() == `pq: password authentication failed for user "postgres"`) {
			r.logger.Error("authentication failed in GetDeadLetterCount", "error", err)
			return 0, fmt.Errorf("database authentication failed: %w", err)
		}
...
func isAuthError(err error) bool {
	...
	return strings.Contains(errStr, "password authentication failed") ||
		...
		strings.Contains(errStr, "pq: password authentication failed")
}