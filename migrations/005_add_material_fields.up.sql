...
ADD COLUMN IF NOT EXISTS material_full_text TEXT;

-- Create indexes for material fields
CREATE INDEX IF NOT EXISTS idx_product_material_composition ON product USING gin (material_composition);
CREATE INDEX IF NOT EXISTS idx_product_material_full_text ON product USING gin (to_tsvector('german', COALESCE(material_full_text, '')));
